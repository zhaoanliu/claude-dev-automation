#!/usr/bin/env node
/**
 * AC Coverage Check — CI gate for the feature pipeline.
 *
 * THE CONTRACT (these string conventions are the system's wiring — they must
 * stay identical across feature-design.yml, feature-implement.yml, the
 * verify-ac action, and this script):
 *   - A linked issue is a *design issue* iff its body contains the literal
 *     marker "<!-- implementation-plan-json -->" (feature-design.yml embeds
 *     the machine-readable plan inside that HTML comment).
 *   - Acceptance criteria live under a "## Acceptance criteria" heading, one
 *     "- [ ]" / "- [x]" checkbox line per criterion. The section ends at the
 *     next "## " heading or an HTML comment.
 *   - A test covers criterion N of issue I iff its it()/test() description
 *     contains the tag "[AC-I-N]" (N is the 1-based checkbox position).
 *
 * What it does, for each design issue linked to the PR via "Closes #N":
 *   - Fail if "## Acceptance criteria" section is missing or has no checkbox items
 *   - Fail if any AC item N has no test tagged [AC-{issue}-{N}] in its
 *     it()/test() description (unit tests AND E2E specs are both scanned)
 *   - Fail if any tagged unit test failed
 *   - Check off passing AC items in the design issue
 *   - Write needs_e2e=true to GITHUB_OUTPUT when any AC items are covered only
 *     by E2E tests, so the calling workflow can gate CI on an E2E run
 *
 * ADAPT for your repo:
 *   - E2E scan directory: scanE2EFiles('e2e') below — change if your Playwright
 *     specs live elsewhere (matches *.spec.ts / *.spec.js recursively).
 *   - RESULTS_FILE: expects Vitest/Jest-compatible JSON reporter output
 *     (testResults[].assertionResults[].{fullName,status}), produced by e.g.
...
for (const { number: issueNum, body } of designIssues) {
  ...
  for (let n = 1; n <= checkboxIndices.length; n++) {
    ...
    if (unitMatches.length === 0 && coveredByE2E) {
      log(`  ✓ [AC-${issueNum}-${n}] — covered by E2E test (E2E gate will be required)`)
      needsE2E = true
...
if (needsE2E) {
  log('\nℹ️  Some AC criteria are covered only by E2E tests — an E2E run is required as a CI gate')
}
